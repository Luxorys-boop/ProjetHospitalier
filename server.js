...
const verifierReposHebdomadaire = (cycle) => {
  let heuresTravaillees = new Array(cycle.length).fill(0);
  // Remplir les heures travaillées par jour
  cycle.forEach((shift, index) => {
    if (!shift.typeRepos) {
      // Ajouter les heures travaillées sur la journée
      heuresTravaillees[index] = shift.duree;
    }
  });
  // Vérifier chaque période de 7 jours glissants
  for (let i = 0; i <= cycle.length - 7; i++) {
    let heuresTotales = 0;
    let heuresReposMax = 0;
    let heuresReposActuelles = 0;
    for (let j = 0; j < 7; j++) {
      let jourIndex = i + j;
      heuresTotales += heuresTravaillees[jourIndex];
      if (heuresTravaillees[jourIndex] === 0) {
        heuresReposActuelles += 24;
      } else {
        heuresReposActuelles = 0;
      }
      heuresReposMax = Math.max(heuresReposMax, heuresReposActuelles);
    }
    // Vérifier si la période contient au moins 36h consécutives de repos
    if (heuresReposMax >= 36) {
      return true; // Contrainte respectée
    }
  }
  return false; // Aucune période de 36h consécutives de repos trouvée
};
...
const verifierHeuresMaxSemaine = (cycle) => {
  let heuresTravaillees = new Array(cycle.length).fill(0);
  // Stocker les heures travaillées par jour
  cycle.forEach((shift, index) => {
    heuresTravaillees[index] = shift.duree;
  });
  // Vérifier chaque période glissante de 7 jours
  for (let i = 0; i <= cycle.length - 7; i++) {
    let totalHeures = 0;
    for (let j = 0; j < 7; j++) {
      totalHeures += heuresTravaillees[i + j];
    }
    if (totalHeures > 44) {
