  ...
  useEffect(() => {
    const fetchUsersCycles = async () => {
      try {
        ...
        // Regrouper les données par user_id et cycle_id
        const groupedCycles = data.reduce((acc, curr) => {
          if (!acc[curr.user_id]) acc[curr.user_id] = {};
          if (!acc[curr.user_id][curr.cycle_id]) acc[curr.user_id][curr.cycle_id] = [];
          acc[curr.user_id][curr.cycle_id][curr.jour - 1] = curr.shift_nom; // Remplit les jours dans le bon ordre
          return acc;
        }, {});
